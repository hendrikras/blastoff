...
export default class Player extends CollidesWithObjects {
    ...
    public update() {
        ...
        this.head.update();
        const { equator, pi2: all, isObscured} = this.head as unknown as SphereClass;
        const {curve: eyeTopLine, isObscured: isTopEyeObscured} = this.head.getSlice('x', 0.5);
        const {curve: eyeBottomLine, isObscured: isBottomEyeObscured} = this.head.getSlice('x', -0.5);
        ...
        let eyeCenter2 = equator.getPoint(eye2Angle);
        let eyeCenter1 = equator.getPoint(eye1Angle);

        const nosePoint = equator.getPoint(nose);
        const noseObscured = isObscured(nosePoint);
        const mouth1Obscured = isObscured(eyeCenter1);
        const mouth2Obscured = isObscured(eyeCenter2);
        // if (mouth1Obscured && !noseObscured) {
        //        eyeCenter1 = eyeCenter1.distance(mouth1Obscured[0]) < eyeCenter1.distance(mouth1Obscured[1]) ? mouth1Obscured[0] : mouth1Obscured[1];
        //     }
        eyeCenter1 = this.getPointOnHead(mouth1Obscured, eyeCenter1);
        // if (mouth2Obscured && !noseObscured) {
        //         eyeCenter2 = eyeCenter2.distance(mouth2Obscured[0]) < eyeCenter1.distance(mouth2Obscured[1]) ? mouth2Obscured[0] : mouth2Obscured[1];
        //     }
        eyeCenter2 = this.getPointOnHead(mouth2Obscured, eyeCenter2);

        // if (!noseObscured) {
                unubscuredShapes.push({type: -2, shape: new QuadraticBezier(eyeCenter1, nosePoint, eyeCenter2), color: 0x00});
            // }
