...
import buildPolysFromGridMap  from '../../navmesh/src/map-parsers/build-polys-from-grid-map';
import { Point, PolyPoints } from "../../navmesh/src/common-types";
/**
...
export default class PhaserNavMeshPlugin extends Phaser.Plugins.ScenePlugin {
  ...
  public boot() {
    const emitter = (this as any).systems.events;
  ...
  public destroy() {
    (this as any).systems.events.off("boot", this.boot, this);
  ...
  public buildMeshFromTilemap(
    ...
    const mesh = new PhaserNavMesh(this, (this as any).scene, key, polygons, 0);
  ...
  public buildMeshfromPolygons(key: string, polygons: PolyPoints[], shrinkAmount = 0) {
    // Offset and scale each polygon if necessary.
    // if (scaleX !== 1 && scaleY !== 1 && offsetX !== 0 && offsetY !== 0) {
    //   polygons = polygons.map((poly) =>
    //     poly.map((p) => ({ x: p.x * scaleX + offsetX, y: p.y * scaleY + offsetY }))
  ...
  public buildMeshFromTiled(
    ...
    const mesh = new PhaserNavMesh(this, (this as any).scene, key, polygons, meshShrinkAmount);
